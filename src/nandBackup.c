...
uint32_t copyFile(char* srcFilePath, const char* targetFilePath) {
	...
	do{
		...
		if (buf_size < 0) {
			logLine("! ERROR : when allocating Buffer.");
			return -99;
		}
...
//					WHBLogPrintf("Write %d,%s", fwrite, targetFilePath);
...
//				WHBLogPrintf("Bytes Copied: %d of %d (%i kB/s)", sizew, sizef,  (uint32_t)(((uint64_t)sizew * 1000) / ((uint64_t)1024 * passedMs)));
			}
		} else {
			WHBLogPrintf("! ERROR : Failed to write rc=%d,%s", ret, targetFilePath);
			IOSUHAX_FSA_CloseFile(fsaFd, srcFd);
			free(pBuffer);
			return -1;
		}
		IOSUHAX_FSA_CloseFile(fsaFd, destFd);
		IOSUHAX_FSA_CloseFile(fsaFd, srcFd);
		IOSUHAX_FSA_ChangeMode(fsaFd, targetFilePath, 0x666);
		free(pBuffer);
	} else {
		WHBLogPrintf("! ERROR : Failed to open file rc=%d,%s", ret, srcFilePath);
		free(pBuffer);
		return -1;
	}
	return 0;
...
uint32_t copyFolder(char* srcFolderPath, const char* targetFolderPath) {
	...
	IOSUHAX_FSA_MakeDir(fsaFd, targetFolderPath, 0x666);
    ...
    while (1) {
        ...
        if (data.info.flags & DIR_ENTRY_IS_DIRECTORY) {
            ...
            if (copyFolder(srcFolderPath, targetPath) != 0) {
                ...
                WHBLogPrintf("! ERROR : %s copy failed", data.name);
                IOSUHAX_FSA_CloseDir(fsaFd, dirH);
                return -2;
            }
    ...
    if (nbFilesCopied != 0) WHBLogPrintf("%d files copied sucessfully", nbFilesCopied);
    return 0;
}
...
uint32_t createNandBackup(bool fullFlag) {
    ...
    for (uint32_t i=0; i<nbPaths; i++) {
        ...
        WHBLogPrintf("> Treating %s...", remoteSubFolders[i]);
        ...
        if ( checkEntry(srcFolder) == 2 ) {
            ...
            if (rc != 0) {
                WHBLogPrintf("! ERROR : copyFolder returned %d", rc);
                WHBLogConsoleDraw();
    ...
    WHBLogConsoleDraw();
    if (grc == 0) logLine("Backup created in wiiu/apps/WiiuFtpServer/NandBackup");
    else logLine("!ERROR : failed to create NAND backup");
    logLine(" ");
...
uint32_t restoreNandBackup() {
    ...
    for (int i=0; i<1; i++) {
        ...
        WHBLogPrintf("> Treating %s...", localSubFolders[i]);
    ...
    WHBLogConsoleDraw();
    if (grc == 0) logLine("NAND network files restored successfully !");
    else logLine("!ERROR : failed to restore NAND backup");
    logLine(" ");
