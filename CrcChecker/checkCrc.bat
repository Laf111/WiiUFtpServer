...
REM : main
    ...
    set "browseFolder="!HERE:"=!\vbs\BrowseFolderDialog.vbs""
    set "StartHidden="!HERE:"=!\vbs\StartHidden.vbs""
    set "StartHiddenWait="!HERE:"=!\vbs\StartHiddenWait.vbs""
    
    set "computeFileCrc="!HERE:"=!\computeFileCrc.bat""
...
REM : functions
    ...
    :checkCrcFolder
        ...
        for /F "delims=~" %%i in ('dir /a-d /s /b !fullFolderPath! 2^>NUL') do (
            ...
            if !nbProc! LSS 24 (
                wscript /nologo !StartHidden! !computeFileCrc! !fullFolderPath! !file!
                set /A "nbProc+=1"
            ) else (
                wscript /nologo !StartHiddenWait! !computeFileCrc! !fullFolderPath! !file!
                set /A "nbProc=0"
            )
            set /A "nbFiles+=1"
        )
